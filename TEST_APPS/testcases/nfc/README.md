...
2. The test app assumes iso-dep level functionality (Type4) and currently hard-codes that assumption into Controller initialization. There is no test case for different tag types.

1. The smartposter NDEF record wrapper class `smartposter.h` is also provided as part of the NFC examples. The examples are not needed to compile the test app, but this example class may be usefull to customers. This file may thus mive  into the NFC component in future.